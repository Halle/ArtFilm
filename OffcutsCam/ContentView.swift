        ...
        VStack {
            ...
            .onChange(of: effect) { tag in
                let result = propertyManager.setPropertyValue(
                    ...
                    to: moods[tag].rawValue as NSString
                )
                logger
                    .debug(
                        "Setting new property value (\"\(propertyManager.getPropertyValue(withSelectorName: propertyManager.mood) ?? "Unknown new string")\") was \(result ? "successful" : "unsuccessful")"
                    )
            }
...
class CustomPropertyManager: NSObject {
    ...
    override init() {
        ...
        effect = MoodName(rawValue: getPropertyValue(withSelectorName: mood) ?? MoodName.bypass.rawValue) ?? MoodName
            .bypass
    }
    ...
    func setPropertyValue(withSelectorName selectorName: CMIOObjectPropertySelector, to value: NSString) -> Bool {
        guard let deviceID = deviceObjectID, var propertyAddress = propertyExists(
            inDeviceAtID: deviceID,
            withSelectorName: selectorName
        ) else {
...
class OutputImageManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate,
    ObservableObject { // Manage receipt of video buffers and conversion into observable CGImage for SwiftUI
